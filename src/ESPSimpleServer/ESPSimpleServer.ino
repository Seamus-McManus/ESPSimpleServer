// @file ESPSimpleServer.ino
// @brief Easy-To-Use Webserver Based off of the ESP32 Webserver Example
// This is a heavily-modified version of the original Example that adds on a few extra features to support plug-and-play use and most common use cases related to web servers.
//
// * Setup a web server
...
// * Newly-Implemented AssetHandler class that automatically handles all GET and HEAD requests from user-uploaded files (for hosting static websites and such)
...
//
// See also README.md for further explanation
//
...
//
// Changelog:
// 6/1/26 First upload/creation of this program
...
//This function is called by the handleListFiles method to help recursively find all files in the system
//This function ignores directorys and just lists all files
void recursiveList(File dir, String &result){
  while (File entry = dir.openNextFile()) {
    if (entry.isDirectory()){
      recursiveList(entry,result);
    }
    else{
    if (result.length() > 4) {
      result += ",\n";
...
void handleListFiles() {
  const String uri = server.uri().c_str();
  TRACE("Uri=%s",uri);
  File dir = fsys->open("/", "r");
  String result;

  result += "[\n";
  recursiveList(dir,result);
...
public:
  //Creates a new AssetHandler that handles a specified directory
  AssetHandler(const String &prefix) : pref(prefix){
    TRACE("Automatic Assset Handler registered for items in directory %s \n",prefix);
  }
   //Creates a new AssetHandler that handles the root directory
  AssetHandler () : pref("/"){
    TRACE("Automatic Assset Handler registered for items in all directories\n");
  }

  //Checks if the AssetHandler can handle a request
  //It can handle HTTP_GET and HTTP_HEAD requests automatically
  bool canHandle(WebServer &server, HTTPMethod requestMethod, const String &uri) override{
    return (requestMethod==HTTP_GET||requestMethod==HTTP_HEAD)&&uri.startsWith(pref);
  }

  //handles HTTP_GET and HTTP_HEAD requests automatically for files within subfolders of the root directory
  bool handle(WebServer &server, HTTPMethod requestMethod, const String &requestUri) override {
    ...
    if (fsys->exists(fName)){
      File file = fsys->open(fName);
      if (file) {

        String type = getDType(fName);

        if (requestMethod==HTTP_HEAD){
        server.sendHeader("Content-Type", type);
        server.sendHeader("Content-length",String(file.size()));
        //TRACE("HTTP_HEAD Success for='%s'\n",requestUri.c_str());
        }
        else{
        ...
        file.close();
        //TRACE("HTTP_GET Success for='%s'\n",requestUri.c_str());
      }
    }

    else {

      //TRACE("HTTP_GET Request Failed for='%s'\n",requestUri.c_str());
...
private:
  String pref;
  //Gets to file data type of a specific uri path
  String getDType(const String &path){
...
void setup(void) {
  ...
  TRACE("Starting ESPSimple WebServer...\n");
  ...
  //Deals with GET and HEAD requests for web assets
